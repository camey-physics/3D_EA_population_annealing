...
void PA_simulation::run(void) {
	...
	while (beta < MAX_BETA) {
		...
		if (beta < 0.1) {
			#pragma omp parallel for shared(pop_array, neighbor_table, bond_table, beta, r_thread)
			for (int m = 0; m < pop_size; m++) {
				int thread = omp_get_thread_num();
				for (int swp = 0; swp < num_sweeps; swp++) {
					pop_array[m].high_T_Met_sweep(beta, r_thread[thread], neighbor_table, bond_table);
				}
				pop_array[m].calc_energy(neighbor_table, bond_table);
			}
		}
			...
			for (int m = 0; m < pop_size; m++) {
				int thread = omp_get_thread_num();
				for (int swp = 0; swp < num_sweeps; swp++) {
					pop_array[m].Met_sweep(beta, r_thread[thread], neighbor_table, bond_table);
				}
				pop_array[m].calc_energy(neighbor_table, bond_table);
			}
		}
